...
if($post){   
  ...
  $mail->addReplyTo($result['email']);

}
}
$query = "select * from tbl_mailbox WHERE cat='orderDelete'";
$post = $db->select($query);
if($post){   
   while($result = $post->fetch_assoc()){

  $mail->isHTML(true);
  $mail->Subject='Ordered Cancelled';
...
$query = "select * from tbl_mailbox WHERE cat='orderApprove'";
$post = $db->select($query);
if($post){   
   while($result = $post->fetch_assoc()){

  $mail->isHTML(true);
  $mail->Subject='Ordered Completed';
  $mail->Body= $result['body'];
}
}
  if ($mail->send()) {
...
<div class="container-fluid">
      ...
      <!--Start Dashboard Content-->
                    ...
                    if (isset($_GET['did'])) {
                    $id=$_GET['did'];
                    $delquery = "delete from tbl_orders where id='$id'";
...
<tr>
                      ...
                      <td><?php echo $fm->formatdate1($result['dates']); ?></td>
                      <td>
                        <?php if ($result['status'] == 'pending') { ?>
